  ...
  >
    <div class="title-container flex-box flex-align-center">
      <!-- <el-button-group class="date-btn-group">
        <el-button
          type="primary"
          :class="{'active':dateType == item.idx}"
    ...
    >
      ...
      >
        <el-row :gutter="20">
          <el-col :span="12">
            <div class="grid-content"> <span>风电场：</span> {{listDetails.powerPlantName}}</div>
          </el-col>
          <el-col :span="12">
...
export default {
  ...
  methods: {
    ...
    handleRowClick(row) {
      ...
      this.$store.commit('CHANGE_POWER_PLANT', row.powerPlantName);
      this.$router.push({ path: '/faultOverviewDetails' });
    },
    ...
    handleDownload() {
      ...
      const params = `key=${this.key}&dateType=${dateType}&startDate=${this.startDate}&endDate=${this.endDate}&chooseDate=${this.chooseDate}&faultType=${this.faultType}&start=${this.pagination.start}&size=${this.pagination.size}`;
